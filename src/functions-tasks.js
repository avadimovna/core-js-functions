...
function getArgumentsCount(funcs) {
  const result = [];
  funcs.forEach((func) => result.push(func.length));
  return result;
}
...
function getPolynom(...coefficients) {
  ...
  return function myFunc(x) {
    return coefficients.reduce((result, coef, index) => {
      return result + coef * x ** (coefficients.length - index - 1);
    }, 0);
  };
}
    ...
    const argsStr = args
      .map((arg) => {
        if (Array.isArray(arg)) {
          return `[${arg.map((a) => JSON.stringify(a)).join(',')}]`;
        }
